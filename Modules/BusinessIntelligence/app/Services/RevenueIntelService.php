...
class RevenueIntelService
{
    ...
    {
        $tids = $this->tid($tenantId);
        $from = $from ?? Carbon::now('Asia/Kolkata')->subDays(30)->startOfDay();
        $to   = $to ?? Carbon::now('Asia/Kolkata');
        ...
        $pipeline = match ($dimension) {
            ...
            'day_of_week' => [
                ['$match' => $match],
                ['$group' => ['_id' => ['$dayOfWeek' => ['date' => '$created_at', 'timezone' => 'Asia/Kolkata']], 'revenue' => ['$sum' => '$grand_total'], 'orders' => ['$sum' => 1]]],
                ['$sort' => ['_id' => 1]],
            ],
            'hour' => [
                ['$match' => $match],
                ['$group' => ['_id' => ['$hour' => ['date' => '$created_at', 'timezone' => 'Asia/Kolkata']], 'revenue' => ['$sum' => '$grand_total'], 'orders' => ['$sum' => 1]]],
                ['$sort' => ['_id' => 1]],
            ],
        ...
        $raw = DB::connection('mongodb')->table('synced_orders')
            ->raw(fn ($col) => $col->aggregate($pipeline));
