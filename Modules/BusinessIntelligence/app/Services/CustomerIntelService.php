...
class CustomerIntelService
{
    ...
    {
        $tids = $this->tid($tenantId);
        $from = Carbon::now('Asia/Kolkata')->subMonths($months)->startOfMonth();
        ...
        $raw = DB::connection('mongodb')->table('synced_customers')
            ->raw(fn ($col) => $col->aggregate([
                ...
                ['$group' => [
                    '_id'   => ['$dateToString' => ['format' => '%Y-%m', 'date' => '$created_at', 'timezone' => 'Asia/Kolkata']],
                    'count' => ['$sum' => 1],
                ]],
                ['$sort' => ['_id' => 1]],
            ]));
    ...
    public function cohortRetention(int $tenantId, int $months = 6): array
    {
        ...
        foreach ($orders as $o) {
            ...
            if ($dt instanceof \MongoDB\BSON\UTCDateTime) {
                $dt = Carbon::instance($dt->toDateTime())->setTimezone('Asia/Kolkata');
            } else {
                $dt = Carbon::parse($dt)->setTimezone('Asia/Kolkata');
            }
            $month = $dt->format('Y-m');
