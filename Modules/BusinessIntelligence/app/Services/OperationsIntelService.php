...
class OperationsIntelService
{
    ...
    {
        return Cache::remember("bi:ops:pipeline:{$tenantId}", now()->addMinutes(15), fn () => $this->computeOrderPipeline($tenantId));
    }

    private function computeOrderPipeline(int $tenantId): array
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::warning('[BI] OperationsIntelService::orderPipeline failed: ' . $e->getMessage());
            return ['stages' => [], 'total_orders' => 0, 'total_value' => 0, 'at_risk_count' => 0, 'at_risk_value' => 0];
        }
    }
    ...
    public function dailyOrderVolume(int $tenantId, int $days = 30): array
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::warning('[BI] OperationsIntelService::dailyOrderVolume failed: ' . $e->getMessage());
            return [];
        }
    }
    ...
    public function activityHeatmap(int $tenantId): array
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::warning('[BI] OperationsIntelService::activityHeatmap failed: ' . $e->getMessage());
            return ['orders' => [], 'events' => [], 'search' => [], 'day_labels' => ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']];
        }
    }
    ...
    public function couponIntelligence(int $tenantId): array
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::warning('[BI] OperationsIntelService::couponIntelligence failed: ' . $e->getMessage());
            return ['coupons' => [], 'total_coupon_orders' => 0, 'total_orders' => 0, 'coupon_usage_rate' => 0, 'total_discount_given' => 0, 'total_coupon_revenue' => 0, 'abuse_suspects' => []];
        }
    }
    ...
    public function paymentAnalysis(int $tenantId): array
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::warning('[BI] OperationsIntelService::paymentAnalysis failed: ' . $e->getMessage());
            return [];
        }
    }
}
