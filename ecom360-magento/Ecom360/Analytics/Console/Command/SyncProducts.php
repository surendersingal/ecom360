...
/**
 * CLI: bin/magento ecom360:sync:products
 */
class SyncProducts extends Command
{
    ...
    {
        $this->setName('ecom360:sync:products')
            ->setDescription('Sync all products to the Ecom360 platform')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $result = $this->dataSync->syncProducts($storeId, function (string $message) use ($output) {
        ...
        $output->writeln(sprintf(
            '<info>✓ Product sync complete — %d synced, %d failed.</info>',
            $synced,
            $failed
        ));
