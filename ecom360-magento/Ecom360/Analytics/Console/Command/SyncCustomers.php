...
/**
 * CLI: bin/magento ecom360:sync:customers
 */
class SyncCustomers extends Command
{
    ...
    {
        $this->setName('ecom360:sync:customers')
            ->setDescription('Sync all customers to the Ecom360 platform')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln('<info>Starting customer sync...</info>');

        $result = $this->dataSync->syncCustomers($storeId, function (string $message) use ($output) {
        ...
        $output->writeln(sprintf(
            '<info>✓ Customer sync complete — %d synced, %d failed.</info>',
            $synced,
            $failed
        ));
